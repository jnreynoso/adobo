...
impl Parser {
        ...
        if let Some(pos) = content.rfind("startxref") {
            ...
            let offset_str = offset_part.split_whitespace().next().ok_or_else(|| {
                io::Error::new(
                    ...
                    "Could not find offset after startxref",
                )
            })?;
            ...
            Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "startxref not found in the last 1024 bytes",
            ))
        }
    }
    ...
    fn parse_xref(&mut self, offset: u64) -> io::Result<()> {
            ...
            loop {
                ...
                if line.trim() == "trailer" {
                    ...
                    if &buf == b"<<" {
                        ...
                        if let PdfObject::Dictionary(map) = trailer_dict {
                            current_trailer = map;
                        }
                    }
                    break;
                        ...
                        (parts[0].parse::<u32>(), parts[1].parse::<u32>())
                    {
                        for i in 0..count {
                            ...
                            if entry_parts.len() == 3 && entry_parts[2] == "n" {
                                ...
                                self.xref
                                    .entry(start_obj + i)
                                    .or_insert(XrefEntry::Normal(obj_offset));
                            }
                        }
                    }
        ...
        for (k, v) in current_trailer.iter() {
            self.trailer.entry(k.clone()).or_insert(v.clone());
        }
    ...
    fn parse_xref_stream_data(
        ...
        let predictor: i64 = if let Some(PdfObject::Dictionary(params)) = dict.get("DecodeParms") {
            if let Some(PdfObject::Integer(p)) = params.get("Predictor") {
                *p
            } else {
                1
            }
        ...
        let unfiltered_data = if predictor >= 10 {
            Self::unfilter_png(data, columns)?
        } else {
            data.to_vec()
        };
        let size = if let Some(PdfObject::Integer(s)) = dict.get("Size") {
            *s as u32
        } else {
            0
        };
        ...
        for chunk in index.chunks(2) {
            ...
            let count = chunk[1];
            for i in 0..count {
                if offset + entry_size > unfiltered_data.len() {
                    break;
                }
                ...
                for &width in &w {
                    let mut val: u64 = 0;
                    for _ in 0..width {
                        val = (val << 8) | (entry[field_offset] as u64);
                        field_offset += 1;
                    }
                    v.push(val);
                }
                ...
                if type_field == 1 {
                    self.xref
                        .entry(start_obj + i)
                        .or_insert(XrefEntry::Normal(v[1]));
                } else if type_field == 2 {
                    self.xref
                        .entry(start_obj + i)
                        .or_insert(XrefEntry::Compressed(v[1] as u32, v[2] as u32));
                }
                offset += entry_size;
            }
        }
        ...
        if let PdfObject::Reference(obj_num, _) = ref_obj {
            ...
            let entry = self.xref.get(obj_num).cloned().ok_or_else(|| {
                io::Error::new(
                    ...
                    format!("Object {} not found in XREF", obj_num),
                )
            })?;
    ...
    fn resolve_maybe_stream<R: Read + Seek>(&mut self, reader: &mut R) -> io::Result<PdfObject> {
        ...
        if reader.read(&mut buf).is_ok() && &buf == b"stream" {
            ...
            if let PdfObject::Dictionary(ref dict) = obj {
                let length_obj = dict.get("Length").ok_or_else(|| {
                    io::Error::new(io::ErrorKind::InvalidData, "Stream missing Length")
                })?;
                let length_val = self.resolve_reference(&length_obj.clone())?;
                let length = if let PdfObject::Integer(l) = length_val {
                    l as usize
                } else {
                    0
                };
    ...
    fn resolve_compressed_object(
        ...
        if let PdfObject::Stream(dict, data) = container {
            let first = if let Some(PdfObject::Integer(f)) = dict.get("First") {
                *f as usize
            } else {
                0
            };
        ...
        if let Some(info_ref) = self.get_info().cloned() {
            ...
            if let PdfObject::Dictionary(info_dict) = info {
                if let Some(title_obj) = info_dict.get("Title") {
                    let title_resolved = self.resolve_reference(title_obj)?;
                    if let PdfObject::String(bytes) = title_resolved {
                        if bytes.starts_with(&[0xFE, 0xFF]) {
                            let utf16: Vec<u16> = bytes[2..]
                                .chunks_exact(2)
                                .map(|c| ((c[0] as u16) << 8) | (c[1] as u16))
            ...
            if let PdfObject::Dictionary(page_dict) = page_obj {
                ...
                while let Some(node) = current_node {
                    ...
                    if let Some(parent_ref) = node.get("Parent") {
                        if let Ok(PdfObject::Dictionary(parent_dict)) =
                            self.resolve_reference(parent_ref)
                        {
            ...
            match type_name {
                Some(PdfObject::Name(n)) if n == "Pages" => {
                    let kids = dict.get("Kids").ok_or_else(|| {
                        io::Error::new(io::ErrorKind::InvalidData, "Pages missing Kids")
                    })?;
                        ...
                        for kid_ref in arr {
                            ...
                            if let PdfObject::Dictionary(kid_dict) = kid {
                                if let Some(PdfObject::Name(n)) = kid_dict.get("Type") {
                                    if n == "Page" {
                                        if target_index == 0 {
                                            return Ok(kid_ref.clone());
                                        }
                                        target_index -= 1;
                                    } else if n == "Pages" {
                                        let count = if let Some(PdfObject::Integer(c)) =
                                            kid_dict.get("Count")
                                        {
                                            *c as usize
                                        } else {
                                        ...
                                        if target_index < count {
                                            return self.find_page_in_tree(kid_ref, target_index);
                                        }
                                        target_index -= count;
                                    }
                                }
                ...
                Some(PdfObject::Name(n)) if n == "Page" => {
                    ...
                    "beginbfchar" => {
                        ...
                        for _ in 0..count {
                            if i + 1 < tokens.len() {
                                ...
                                {
                                    if let Some(c) = std::char::from_u32(d) {
                                        map.insert(s, c);
                                    }
                                }
                                i += 2;
                    ...
                    "beginbfrange" => {
                        ...
                        for _ in 0..count {
                            if i + 2 < tokens.len() {
                                ...
                                if let (Ok(ss), Ok(es), Ok(d)) = (
                                    u32::from_str_radix(start_src, 16),
                                    u32::from_str_radix(end_src, 16),
                                    u32::from_str_radix(dst, 16),
                                ) {
                                    for offset in 0..=(es - ss) {
                                        if let Some(c) = std::char::from_u32(d + offset) {
                                            map.insert(ss + offset, c);
                                        }
                                    }
                                }
                                i += 3;
    ...
    pub fn get_root(&self) -> Option<&PdfObject> {
        self.trailer.get("Root")
    }
    pub fn get_info(&self) -> Option<&PdfObject> {
            ...
            if b[0].is_ascii_whitespace() {
                ...
                if s == "obj" {
                    ...
                    if reader.read(&mut next).is_ok() {
                        if next[0].is_ascii_whitespace() || next[0] == b'<' {
                            reader.seek(SeekFrom::Current(-1))?;
                            return Ok(());
                        }
                    } else {
    ...
    fn parse_object<R: Read + Seek>(reader: &mut R) -> io::Result<PdfObject> {
        ...
        match b[0] {
            b'/' => {
                reader.seek(SeekFrom::Current(-1))?;
                Ok(PdfObject::Name(Self::parse_name(reader)?))
            }
            b't' | b'f' | b'n' => {
                reader.seek(SeekFrom::Current(-1))?;
                Self::parse_keyword_or_bool(reader)
            }
            b'0'..=b'9' | b'-' => {
                reader.seek(SeekFrom::Current(-1))?;
                Self::parse_number_or_reference(reader)
            }
            b'<' => {
                let mut next = [0; 1];
                reader.read_exact(&mut next)?;
                if next[0] == b'<' {
                    Self::parse_dictionary_content(reader)
                } else {
                    Self::parse_hex_string(reader, next[0])
                }
            }
            ...
            _ => Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("Unexpected char: {}", b[0] as char),
            )),
        }
    }
    ...
    fn parse_number_or_reference<R: Read + Seek>(reader: &mut R) -> io::Result<PdfObject> {
        ...
        let mut b = [0; 1];
        while reader.read(&mut b)? > 0 {
            if b[0].is_ascii_digit() || b[0] == b'.' || b[0] == b'-' {
                s.push(b[0] as char);
            } else {
                reader.seek(SeekFrom::Current(-1))?;
                break;
        ...
        if !s2.is_empty() {
            Self::skip_whitespace(reader)?;
            if reader.read(&mut b)? > 0 && b[0] == b'R' {
                return Ok(PdfObject::Reference(
                    s.parse().unwrap(),
                    s2.parse().unwrap(),
                ));
            }
        ...
        reader.seek(SeekFrom::Start(pos))?;
        if s.contains('.') {
            Ok(PdfObject::Real(s.parse().unwrap()))
        } else {
            Ok(PdfObject::Integer(s.parse().unwrap()))
        }
    }
        ...
        for row in data.chunks_exact(row_size) {
            ...
            for i in 0..columns {
                ...
                match filter_type {
                    ...
                    3 => {
                        current_row[i] =
                            row_data[i].wrapping_add(((left as u16 + up as u16) / 2) as u8)
                    }
                    4 => {

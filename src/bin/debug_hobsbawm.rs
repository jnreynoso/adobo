...
fn main() {
    ...
    let default_font = std::sync::Arc::new(ab_glyph::FontVec::try_from_vec(
        std::fs::read("C:/Windows/Fonts/arial.ttf").unwrap()
    ).unwrap());
    ...
    let interpreter = ufreader::interpreter::Interpreter::new(font_encodings, font_widths, std::collections::HashMap::new(), font_names);
    ...
    for i in 0..10.min(count as usize) {
        ...
        let page_rect = parser.get_page_rect(i).unwrap_or(ufreader::parser::PageRect { x: 0.0, y: 0.0, width: 595.0, height: 842.0 });
        ...
        if let Some(cmds) = interpreter.process(i, &content, page_rect, Some(&epoch), Some(0)) {
                ...
                if let ufreader::interpreter::DrawCommand::Text { chars, size, .. } = cmd {
                    ...
                    for (c, x, expected_w) in chars {
                        ...
                        if let Some(outline) = font.outline(glyph_id) {
                            ...
                            let h_squeeze = if actual_w > 0.0 && expected_w > 0.0 { expected_w / actual_w } else { 1.0 };
                            ...
                            for curve in outline.curves {
                                ...
                                if is_new_contour {
                                    builder.move_to(((x + start_p.x * scale_factor * h_squeeze) as f64, (start_p.y * scale_factor) as f64));
                                }
                                match curve {
                                    ab_glyph::OutlineCurve::Line(_, p2) => {
                                        builder.line_to(((x + p2.x * scale_factor * h_squeeze) as f64, (p2.y * scale_factor) as f64));
                                        last_point = Some(p2);
                                    }
                                    ab_glyph::OutlineCurve::Quad(_, p2, p3) => {
                                        builder.quad_to(((x + p2.x * scale_factor * h_squeeze) as f64, (p2.y * scale_factor) as f64), ((x + p3.x * scale_factor * h_squeeze) as f64, (p3.y * scale_factor) as f64));
                                        last_point = Some(p3);
                                    }
                                    ab_glyph::OutlineCurve::Cubic(_, p2, p3, p4) => {
                                        builder.curve_to(((x + p2.x * scale_factor * h_squeeze) as f64, (p2.y * scale_factor) as f64), ((x + p3.x * scale_factor * h_squeeze) as f64, (p3.y * scale_factor) as f64), ((x + p4.x * scale_factor * h_squeeze) as f64, (p4.y * scale_factor) as f64));
                                        last_point = Some(p4);
                                    }
                                }
                            }
            ...
            println!("Page {} text cmds: {}, chars: {}, outlines: {}", i, text_count, total_chars, outline_count);
        } else {
